                ...
                <div className="available-wallets">
                    <div className="wallet-preview">
                        ...
                        userInfo.wallets.map(item => 
                            <div onClick={() => {setActiveWallet(item);setWithdrawError(false);setWithdrawSuccess(false)}} className="wallet-preview-chain">
                                <div style={{display: "flex", alignItems: "center", width: "50%"}}>
                                    ...
                                    <div style={{display: "flex", flexDirection: "column", width: "100%"}}>
                                        <h3>{item.chain} Wallet</h3>
                                        <p style={{color: "darkgrey", textOverflow: "ellipsis", overflow: "hidden", whiteSpace: "nowrap", width: "100%"}}>{item.address}...</p>
                                    </div>
                                </div>
                                <p>{parseFloat(item.balance.$numberDecimal).toFixed(2)} {item.chain}</p>
                            </div>
                    ...
                    <div className="wallet-card">
                        <div className="wallet-card-header">
                            <div style={{display: "flex", alignItems: "center"}}>
                                <img style={{height: "80px", width: "80px", marginRight: "20px"}} src={require(`../../static/assets/SVG-icons/${(activeWallet.chain).toLowerCase()}.svg`)} />
                                <div>
                                    <h1>{activeWallet.chain} Wallet</h1>
                                    <p style={{color: "darkgrey", fontSize: "18px"}}>{supportedCoinsNames[activeWallet.chain]}</p>
                                </div>
                            </div>
                            <h2>{parseFloat(activeWallet.balance.$numberDecimal).toFixed(4)} {activeWallet.chain}</h2>
                        </div>
                        <div className="wallet-card-info">
                            <h2 style={{fontWeight: "300", marginBottom: "20px", color: "lightblue"}}>Deposit</h2>
                            <div style={{display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: "30px"}}>
                                <h4 style={{color: "darkgray", fontSize: "17px"}}>Address : <span style={{color: "white", fontWeight: "200"}}>{activeWallet.address}</span></h4>
                                <button onClick={openModal} className="coin-market-trade-but">View QRcode</button>
                            </div>
                            <h2 style={{fontWeight: "300", marginBottom: "20px", color: "lightblue"}}>Withdraw</h2>
                            <div style={{display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: "20px"}}>
                                <p style={{color: "darkgray"}}>From:</p>
                                <input value={activeWallet.address} readOnly className="wallets-form-input" />
                                <span style={{fontSize: "36px"}} class="material-icons">
                                    swap_horiz
                                </span>
                                <p style={{color: "darkgray"}}>To:</p>
                                <input value={withdrawForm.targetAddress} onChange={onChange} name="targetAddress" className="wallets-form-input" />
                            ...
                            <div style={{display: "flex", alignItems: "center", marginBottom: "30px"}}>
                                ...
                                <input value={withdrawForm.amount} onChange={onChangeFiltered} name="amount" style={{width: "15%", textAlign: "center", margin: "0px 10px"}} className="wallets-form-input" />
                            ...
                            ) : <p style={{fontSize: '18px'}}>Empty Transaction Record.</p>
                            }
                        </div>
                    </div>
                    }
